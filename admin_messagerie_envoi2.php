...
<?php 
     $id_ab=$_GET['id'];
	 ?>
<div class="titre"> <img style="vertical-align: middle;" src='image/mail.png'/> Contacter abonnée </div>
 <div class="form_parametre">
	   ...
	   <div class="div_messagerie3">
	    <form name="form" method="post" <?php echo'action="admin_messagerie_envoi_traitement1.php?id='.$id_ab.'" ';?> onSubmit="return verification()">
		            ...
		            <td width=50%>
					    <select name="pseudo" id="pseudo" >
						 ...
						 <?php
							...
							$reponse = mysql_query("SELECT * FROM candidat where idc='".$id_ab."'");
                            while ($rep = mysql_fetch_array ($reponse))
                            {
							echo '<option value="'.$rep['pseudo'].'" >'.$rep['pseudo'].'</option>';
