...
<head>
<title>Messagerie Admin consultation</title>
...
$reponse = mysql_query("SELECT * FROM messagerie_admin where idmsga='$id_messag' and supprimer_admin='false' ");
while ($rep = mysql_fetch_array ($reponse))
{
$pseu=$rep['pseudo_candidat'];
?>
	   ...
	   <div class="div_messagerie2">
		...
		<center><a style="text-decoration: none;" href="admin_messagerie.php"><input type="submit" value="Fermer" /></a> <?php echo' <a style="text-decoration: none;" href="admin_messagerie_envoi1.php?ps='.$pseu.'"><input type="submit" value="Répondre" /></a>';?></center>
		   <?php
}
//pour la mise a jour des message non lu
 $reponse=mysql_query("select * from messagerie_admin where idmsga='$id_messag'");
            $res=mysql_fetch_row($reponse);
            $existe=mysql_num_rows($reponse);
	  
	         if ($existe)
                      {
					  mysql_query("UPDATE messagerie_admin SET lu='true' where idmsga='$id_messag'");
					  }
